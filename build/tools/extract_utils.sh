...
function setup_vendor() {
    ...
    export ZEUS_ROOT="$3"
    if [ ! -d "$ZEUS_ROOT" ]; then
        echo "\$ZEUS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$ZEUS_ROOT/$OUTDIR" ]; then
        mkdir -p "$ZEUS_ROOT/$OUTDIR"
    fi

    export PRODUCTMK="$ZEUS_ROOT"/"$OUTDIR"/"$DEVICE"-vendor.mk
    export ANDROIDMK="$ZEUS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$ZEUS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$ZEUS_ROOT"/"$OUTDIR"/proprietary
    if [ "$SRC" = "adb" ]; then
        init_adb_connection
    fi
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            cp "$SRC/$FILE" "$DEST"
            # if file does not exist try ZEUS target
            if [ "$?" != "0" ]; then
                cp "$SRC/$TARGET" "$DEST"
            fi
        fi
